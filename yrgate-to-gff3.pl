...
sub mrna_to_gff3
{
  ...
  $gff3[8] .= $mrna->{"proteinAliases"} eq ""
              ? "" : ";Alias=". $mrna->{"proteinAliases"};
  $gff3[8] .= $mrna->{"proteinId"} eq ""
              ? "" : ";description=\"". $mrna->{"proteinId"} ."\"";
  $gff3[8] .= $mrna->{"USERid"} eq ""
              ? "" : ";contributor=\"". $mrna->{"USERid"} ."\"";
  print $OUT join("\t", @gff3), "\n";
...
while(my $annot = $query->fetchrow_hashref())
{
  ...
  if($class ne "Delete" and $class ne "Not Resolved" and $class ne "Confirm" and $class ne "Variant")
  {
